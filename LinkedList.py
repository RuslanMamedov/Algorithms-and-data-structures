...
node4.next = node5
node5.next = node6
#6. Reverse a linked list reccursively
...
def ReverseIter(head):
    ...
    while p2!= None:
        ...
        p2=p2.next
    return p3
print ('#6.',ReverseIter(node1))
#recursive approach - starting from the tail
def ReverseRec (head):
    if head==None or head.next == None:
        return head
    prev = ReverseRec(head.next)
    head.next.next = head
    head.next = None
    return prev
print ('#6.')
p=ReverseRec(node6) #reversing the reversed linked list
while p!=None:
    print (p.data)
    p=p.next
